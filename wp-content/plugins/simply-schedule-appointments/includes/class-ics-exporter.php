...
class SSA_Ics_Exporter {
	...
	protected function get_file_url( $filename ) {
		...
		return SSA_Bootstrap::maybe_fix_protocol( $url . '/' . sanitize_title( $filename ) . '.ics' );
	}
	...
	protected function generate() {
		...
		foreach ( $this->appointments as $appointment ) {
			...
			$web_meeting_url = $appointment->__get( 'web_meeting_url' );
			if ( ! empty( $web_meeting_url ) ) {
				$ics .= 'LOCATION:' . $web_meeting_url . $this->eol;
			} else {
