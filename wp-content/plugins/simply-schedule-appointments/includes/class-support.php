...
class SSA_Support {
	...
	public function bulk_send_notifications() {
		...
		$customer_booked_notification = array_values(
			array_filter(
				$notifications,
				function( $notification ) {
					return 'appointment_booked' === $notification['trigger'] && strpos( implode( ';', $notification['sent_to'] ), 'customer_email' ) !== false;
				}
			)
		);

		// If notification is found, then send list of appointments to validate the notification and possibly send them.
		if ( ! empty( $customer_booked_notification ) ) {
			$this->plugin->action_scheduler->bulk_schedule_notifications( $customer_booked_notification[0], $appointments );
		}
