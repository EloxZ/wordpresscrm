...
function zeroBSCRM_pages_systemstatus() {
    jpcrm_load_page( 'system', 'main' );
}

// Email Box
function zeroBSCRM_pages_emailbox() {
  // load
    jpcrm_load_page( 'email', 'main' );
  // render
...
function zeroBSCRM_pages_admin_your_profile(){
  ...
  jpcrm_load_page( 'user-profile', 'main' );
...
function zeroBSCRM_pages_admin_reminders(){
  ...
  jpcrm_load_page( 'user-profile', 'reminders' );
  ...
  <div id="zbs-welcome">
    ...
    <div class="container margin-top30">
      ...
      <div class="upgrade-cta upgrade">
                ...
                <li><span class="dashicons dashicons-yes"></span> WooCommerce Connect</li>
              ...
              <div class="zbs-button-wrap">
							<a href="<?php echo $zbs->urls['upgrade']; ?>" rel="noopener noreferrer" target="_blank" class="btn btn-cta">
								Buy Now</a>
              </div>
						</div>
            <div class="clear"></div>
...
function zeroBSCRM_html_installextensionshelper(){
  ...
  ?>
          ...
          <p class="intro"><?php echo sprintf( __( 'To control which modules and extensions are active, please go the <a href="%s">Extension Manager</a>.', 'zero-bs-crm' ), admin_url( 'admin.php?page='.$zbs->slugs['extensions'] ) );?></p>
...
function zeroBSCRM_extensions_init_install(){
    ...
    if ( isset( $_GET['zbsinstall'] ) && ! empty( $_GET['zbsinstall'] ) ) {
      ...
      if (
        ...
        wp_redirect( zbsLink( $zbs->slugs['extensions'] ) );
...
function zeroBSCRM_html_extensions(){
  ...
  #} Install msg
  if ( !empty( $extension_messages ) ){
    echo '<div class="zbs-page-wrap install-message-list">';
    if ( $extension_messages['success'] ){


      $msgHTML = '<i class="fa fa-check" aria-hidden="true"></i> Successfully '.$extension_messages['action'].'ed extension: '.$extension_messages['pretty_name'];

      // if API, catch and give further info (e.g. no key)
      if ($extension_messages['action'] == 'install' && $extension_messages['pretty_name'] == 'API'){
      ...
      if ($extension_messages['action'] == 'install'){

        $msgHTML .= '<br /><i class="fa fa-info-circle" aria-hidden="true"></i> <a href="'.$extension_messages['helpurl'].'" target="_blank">View '.$extension_messages['pretty_name'].' Help Documentation</a>';
      ...
      $errmsg = 'Unable to install extension: '.$extension_messages['pretty_name'].', please contact <a href="'.$zbs->urls['support'].'" target="_blank">Support</a> if this persists.';

      if ( isset( $extension_messages['error_msg'] ) ) $errmsg .= '<br />Installer Error: '.$extension_messages['error_msg'];
