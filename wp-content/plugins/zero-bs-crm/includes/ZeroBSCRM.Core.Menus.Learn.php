...
require_once( ZEROBSCRM_INCLUDE_PATH . 'ZeroBSCRM.Core.Menus.Learn.Links.php' );
...
function zeroBSCRM_admin_subtop_menu(){
	...
	switch ($slug){
		...
		case $zbs->slugs['manage-events']:
			...
			break;
		case $zbs->slugs['manage-events-list']:
			zeroBSCRM_tasklistview_learn_menu(); $menuOut = true;
			break;
...
function zeroBS_genericLearnMenu($title='',$addNew='',$filterStr='',$showLearn=true,$h3='',$learnContent='',$learnMoreURL='',$learnImgURL='',$learnVidURL=false,$hopscothCustomJS=false,$popupExtraCSS=''){
    ...
    <div id="zbs-admin-top-bar" style="margin-bottom:-20px;">
      <div id="zbs-list-top-bar">
          <div class="zbs-white"><span class="add-new-button"><?php echo $title; ?></span><?php if (!empty($addNew)) echo ' '.$addNew; ?>
          	<?php if ($showLearn){ ?>
...
function zeroBS_generateLearnLinks($key=''){
    global $zbs, $zbs_learn_links_array, $zbs_learn_img_array, $zbs_learn_video_link_array;
...
function zeroBSCRM_tasklistview_learn_menu(){
