...
function zeroBSCRM_admin_menu() {
	...
	if ($zbsMenuMode == ZBS_MENU_SLIM) $adminMenuTitle = 'CRM ' . __('Settings',"zero-bs-crm");
    ...
    $adminMenuWelcome = add_submenu_page( $zbs->slugs['home'], __('Welcome to JPCRM',"zero-bs-crm"), __('Welcome to JPCRM',"zero-bs-crm"), 'admin_zerobs_manage_options', $zbs->slugs['home'], 'zeroBSCRM_pages_home' );
	...
	$adminManageT = add_submenu_page( null, __('Send Email',"zero-bs-crm"), __('Send Email',"zero-bs-crm"), 'admin_zerobs_customers', $zbs->slugs['sendmail'], 'zeroBSCRM_pages_emailsend' );
	add_action( "admin_print_styles-{$adminManageT}", 'zeroBSCRM_global_admin_styles' );
	add_action( "admin_print_styles-{$adminManageT}", 'zeroBSCRM_email_styles' );

	#} new EMAILS page
	$adminEmails = add_submenu_page( null, __('Emails',"zero-bs-crm"), __('Emails',"zero-bs-crm"), 'admin_zerobs_customers', $zbs->slugs['emails'], 'zeroBSCRM_pages_emailbox' );
    ...
    #} System Status
    $adminMenuStatus = add_submenu_page( $zbs->slugs['home'], __('System Assistant',"zero-bs-crm"), __('System Status',"zero-bs-crm"), 'admin_zerobs_manage_options', $zbs->slugs['systemstatus'], 'zeroBSCRM_pages_systemstatus' );
...
//if dal 2}
	...
	if ($ZBSuseCalendar == "1" && zeroBSCRM_permsEvents()){
	    ...
	    $adminManageE = add_submenu_page( 'edit.php?post_type=zerobs_event', __('Task Scheduler',"zero-bs-crm"), __('Task Scheduler',"zero-bs-crm"), 'admin_zerobs_events', $zbs->slugs['manage-events'], 'zeroBSCRM_render_eventscalendar_page' );
    ...
    if ($zbsMenuMode == ZBS_MENU_SLIM){
		...
		*/
    	...
    	//extensions - can do manually (only mail campaigns and sales dashboard exist after merge in of PayPal + WooCommerce Connect + CSV importer)
		...
		$adminUserD = add_menu_page( __('Jetpack CRM User Dash',"zero-bs-crm"), 'Jetpack CRM', 'zbs_dash', $zbs->slugs['dash'], 'zeroBSCRM_pages_dash','dashicons-groups',2);
		add_action( "admin_print_styles-{$adminUserD}", 'zeroBSCRM_global_admin_styles' );
		...
		if($ZBSuseCalendar == "1"){
		    $adminManageE = add_submenu_page( $zbs->slugs['dash'], __('Task Scheduler',"zero-bs-crm"), __('Task Scheduler',"zero-bs-crm"), 'admin_zerobs_view_events', $zbs->slugs['manage-events'], 'zeroBSCRM_render_eventscalendar_page' );
			add_action( "admin_print_styles-{$adminManageE}", 'zeroBSCRM_global_admin_styles' );
			add_action( "admin_print_styles-{$adminManageE}", 'zeroBSCRM_calendar_admin_styles' );
		}
    	...
    	/* EXTRA ITEMS FOR CAPABILITIES TO SLIM MENU (e.g. PAYPAL SYNC ETC) */
		    ...
		    if (current_user_can( 'manage_options' )){
			      ...
			      if (zeroBSCRM_isExtensionInstalled('woo')){ 
			      	$submenu[$zbs->slugs['dash']][] = array(__('WooCommerce Connect',"zero-bs-crm"),'edit_pages','admin.php?page='.$zbs->slugs['woosync'],__('WooCommerce Connect',"zero-bs-crm"));
			      }
    ...
    if ($zbsMenuMode == ZBS_MENU_FULL){
		...
		$adminUserD = add_menu_page( __('Jetpack CRM User Dash',"zero-bs-crm"), __('CRM Dashboard',"zero-bs-crm"), 'zbs_dash', $zbs->slugs['dash'], 'zeroBSCRM_pages_dash','dashicons-groups',2);
	    add_action( "admin_print_styles-{$adminUserD}", 'zeroBSCRM_global_admin_styles' );
		add_action( "admin_print_styles-{$adminUserD}", "zeroBSCRM_admin_styles_chartjs");
		add_action( "admin_print_styles-{$adminUserD}", "zeroBSCRM_admin_styles_homedash");
