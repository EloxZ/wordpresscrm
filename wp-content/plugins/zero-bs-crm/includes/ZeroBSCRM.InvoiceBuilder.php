    ...
    if (!empty($invoicePostID)){
        ...
        if (!empty($templatedHTML)){
            ...
            $html = $placeholder_templating->replace_placeholders( array( 'invoice', 'global', 'contact', 'company' ), $templatedHTML, array(
                ...
                'invoice-customer-info'         => $custInfoTable,
                ...
                'invoice-label-inv-from'        => __( 'From:', 'zero-bs-crm' ),
                'invoice-label-inv-to'          => __( 'To:', 'zero-bs-crm' ),
                'invoice-label-inv-due-date'    => __( 'Due date:', 'zero-bs-crm' ),
...
function zeroBSCRM_invoice_generatePortalInvoiceHTML($invoicePostID=-1,$return=true){
    ...
    if (!empty($invoicePostID)){
        ...
        //deny access if current user cannot view invoice.
        if(!jpcrm_can_current_user_view_invoice($invoicePostID)){
            return __("Either this invoice does not exist or you do not have permission to view it.", "zero-bs-crm");
        }else{
        ...
        if (!empty($templatedHTML)){
            ...
            #} Mikes previous invoice.php worked into vars:
                ...
                $bizInfoTable = '';
                    ...
                    $custInfoTable .= '<div class="zbs-portal-label">' . __('Invoice To', 'zero-bs-crm') . '</div>';
                ...
                if ($zbs_stat != 'Paid') {
                    ...
                    // so the action below will fire in WooCommerce Connect, and remove the three filters below
...
function zeroBSCRM_invoicing_generateInvoiceHTML($invoiceID=-1,$template='pdf',$html=''){
    ...
    if ( !jpcrm_can_current_user_view_invoice( $invoiceID ) || !$invoice ) {
        return __('Either this invoice does not exist or you do not have permission to view it.', 'zero-bs-crm');
    }
    ...
    #} == Template -> HTML build
            ...
            $replacements = array(
                ...
                'invoice-label-inv-date'        => __( 'Invoice Date:', 'zero-bs-crm' ),
                'invoice-label-inv-ref'         => $zbs->settings->get('reflabel'),
                'invoice-label-inv-from'        => __( 'Fro,:', 'zero-bs-crm' ),
                'invoice-label-inv-to'          => __( 'To:', 'zero-bs-crm' ),
                'invoice-label-inv-due-date'    => __( 'Due date:', 'zero-bs-crm' ),
                'invoice-pay-terms'             => __( 'Payment Terms: Due ', 'zero-bs-crm' ),
    ...
    $custInfoTable = '<div class="customer-info-wrapped">';
        ...
        switch ($template){
            ...
            case 'portal':
                ...
                $custInfoTable .= '<div class="pay-to">';
                    $custInfoTable .= '<div class="zbs-portal-label">' . __('Invoice To', 'zero-bs-crm') . '</div><div style="margin-top:18px;">&nbsp;</div>';
                    $custInfoTable .= '<div class="zbs-portal-biz">';
                        if (isset($invTo['fname']) && isset($invTo['fname'])) $custInfoTable .= '<div class="pay-to-name">'.$invTo['fname'].' ' .$invTo['lname'] . '</div>';
                        if (isset($invTo['addr1'])) $custInfoTable .= '<div>'.$invTo['addr1'].'</div>';
                        if (isset($invTo['addr2'])) $custInfoTable .= '<div>'.$invTo['addr2'].'</div>';
