...
/*
* Checks whether current user has permissions to view invoice
*/
function jpcrm_can_current_user_view_invoice($invID=-1){
    ...
    //also if access by ID then can skip this part
    if ($useHash == "1" && $invHash != ''){
		...
		// ============== / Brute force blocking
				...
				if ($hashOK['success'] == 1){
					...
					if (is_user_logged_in() && $invID > 0){
						...
						if ( $cID && $contactID == $cID || zeroBSCRM_permsInvoices()){
		...
		if ($invID > 0){
			...
			//non-users should only access if easy-access is on and if it's hash
			if ( !$uid ) {
				return false;
			}
			$uinfo = get_userdata( $uid );
...
/*
* Checks whether current user has permissions to view a quote
*/
function jpcrm_can_current_user_view_quote( $quote_id = -1 ){
    ...
    if ($useHash == "1" && $quote_hash != ''){
		...
		// ============== / Brute force blocking
				...
				// log request (start)
				$requestID = zeroBSCRM_security_logRequest('quoeasy',$quote_hash,$quote_id);
				...
				if ($hashOK['success'] == 1){
					...
					// all checks out 
					// ... this has been accessed via clients/quotes/bF6wj0pGO74eXQpYIQZ
					...
					// fall back to showing on the page
					// (if has quote_id + logged in)
					if (is_user_logged_in() && $quote_id > 0){

						$uid = get_current_user_id();
						$uinfo = get_userdata( $uid );
						$cID = zeroBS_getCustomerIDWithEmail($uinfo->user_email);
						$contactID = zeroBSCRM_quote_getContactAssigned((int)$quote_id);

						if ($contactID == $cID || zeroBSCRM_permsQuotes()){

							// ... this has been accessed via clients/quotes/123
		...
		// Normal mode
		// ... this should have been been accessed via clients/quotes/123

		// got quote id at least?
		if ($quote_id > 0){
			...
			if ($contactID == $cID || zeroBSCRM_permsQuotes()){
				//we are admin with manage quote perms can also view
				return true;
			}
		}
	}
