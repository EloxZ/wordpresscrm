...
function zeroBS_portalnav($menu = 'dashboard'){
	...
	foreach ( $nav_items as $menu_name => $menu_data ) {
		...
		echo "<li class='".$class."'><a href='" . $link ."'><i class='fa ".$menu_data['icon']."'></i>". __($menu_data['name'],'zero-bs-crm') . "</a></li>";
...
#} New functions here. Used as NAMED in WooCommerce Connect. Please do not rename and not tell me as need to update WooCommerce Connect if so
	...
	if($cID > 0){
		...
		if(count($customer_invoices) > 0){
			...
			foreach($customer_invoices as $cinv){
				...
				echo '<tr>';
					echo '<td>'. $idLinkStart.$idStr . ' '. __('(view)', 'zero-bs-crm') . $idLinkEnd.'</td>';
		...
		if(count($customer_invoices) > 0){
			...
			foreach($customer_invoices as $cinv){
				...
				*/
				...
				echo '<tr>';
					echo '<td><a href="'. esc_url($link .  $endpoint .  '/' . $cinv['id']) .'">#'. $cinv['zbsid'] . __(' (view)', 'zero-bs-crm') . '</a></td>';
...
function jpcrm_portal_single_quote( $quote_id = -1, $quote_hash='' ){
    ...
    if ($zbs->isDAL3()){
        ...
        // dal3
            ...
            // content
            if (isset($quote_data) && isset($quote_data['content'])) $quote_content = $quote_data['content'];

            // hash (if not passed)
            if (isset($quote_data) && isset($quote_data['hash'])) {
            ...
            //  acceptable?
            if (isset($quote_data) && (!isset($quote_data['accepted']) || (isset($quote_data['accepted']) && empty($quote_data['accepted'])))) {
                $acceptable = true;
            } else {
                
                // setting this shows it at base of quote, when accepted
                if ($quote_data['accepted'] > 0) $acceptedDate = $quote_data['accepted_date'];
            }
        ...
        // pre dal3
            ...
            // content
            if (isset($quote_data) && isset($quote_data['quotebuilder']) && isset($quote_data['quotebuilder']['content'])) $quote_content = $quote_data['quotebuilder']['content'];

            //  acceptable?
            if (isset($quote_data) && isset($quote_data['meta']) && (!isset($quote_data['meta']['accepted']) || (isset($quote_data['meta']['accepted']) && empty($quote_data['meta']['accepted'])))) $acceptable = true;
    ...
    <div id="zerobs-proposal-<?php echo $quote_id; ?> main" class="zerobs-proposal entry-content hentry" style="margin-bottom:50px;margin-top:0px;">
        ...
        <?php if ($acceptable){ ?>
            ...
            <?php 
                ...
                echo '<div id="zbs-quote-accepted-'.$quote_id.'" class="alert alert-success" style="display:none;margin-bottom:5em;">' . __w("Quote accepted, Thank you.","zero-bs-crm"). "</div>";
                echo '<div id="zbs-quote-failed-'.$quote_id.'" class="alert alert-warning" style="display:none;margin-bottom:5em;">' . __w("Quote could not be accepted at this time.","zero-bs-crm"). "</div>";
            ?>
            ...
            <div class="zerobs-proposal-actions" id="zerobs-proposal-actions-<?php echo $quote_id; ?>">
                ...
                <form id="accept-proposal" method="POST">
                    <input type="hidden" id="zbs-quote-id" name="zbs-quote-id" value="<?php echo $quote_id; ?>" />
                    <input type="hidden" id="zbs-quote-hash" name="zbs-quote-hash" value="<?php echo $quote_hash; ?>" />
    ...
    <?php ?>
    <script type="text/javascript" src="<?php echo ZEROBSCRM_URL.'js/ZeroBSCRM.public.proposals'.wp_scripts_get_suffix().'.js?ver=' . $zbs->version; ?>"></script>
    <?php
