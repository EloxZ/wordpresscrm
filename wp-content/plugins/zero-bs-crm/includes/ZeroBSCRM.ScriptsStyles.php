...
function zeroBSCRM_scriptStyles_initStyleRegister(){
	...
	global $zbs;
		...
		// ================  Global  =================
			...
			wp_register_style('semanticui', 	plugins_url('/css/semantic.min.css',ZBS_ROOTFILE), array(), $zbs->version );
		...
		// ============  Libs  =======================
			...
			// jq modal
			wp_register_style('zerobsjsmodal', 	plugins_url('/css/jquery.modal.min.css',ZBS_ROOTFILE), array(), $zbs->version );
...
function zeroBSCRM_calendar_admin_styles(){
	...
	wp_enqueue_style( 'fullcalendar', untrailingslashit(ZEROBSCRM_URL) .'/css/fullcalendar.min.css', array(), $zbs->version );
	...
	wp_register_script('zerobscrm-calendar-js' , ZEROBSCRM_URL .'js/fullcalendar.mod.min.js', array('jquery','wh-moment-v2-8-1-js'), $zbs->version );//2.97.9 upgraded moment array('jquery','wh-moment-v2-1-3-js'));
